...
**这不是本项目特有的问题。** 市面上机制最接近的 **[BTunnel](https://github.com/BarronDEV/btunnel)** 用了**完全不同的 SW↔页面通道设计**（转移 `MessagePort`，而不是 `postMessage`），却也**独立演化出了同一套恢复机制**：它的 `sw.js` 同样把隧道端口存在模块级变量里，页面定时发 `PING_TUNNEL`，SW 一旦发现端口没了就广播 `REQUEST_TUNNEL_PORT`，让页面重新交接。

> 换句话说：**换掉通道设计并不能绕开这个坑**——它来自"SW 会被回收"这件事本身，
> 而不是某一种通道实现的缺陷。这正是这一条值得单独写下来的原因。
...
### 2.17 目标是 TLS-only 时，失败长得像"网络卡了"

| | |
|---|---|
| **症状** | 隧道连上了，但每个请求都返回 502，正文是 `invalid HTTP response: no header terminator`；或者浏览器里干脆是重定向死循环 |
| **原因** | **agent 到目标那一段默认是明文。** 把明文 HTTP 灌进 TLS 监听端口，服务器听不懂就挂了电话——而那段字节里当然永远不会有 `\r\n\r\n` |
| **修法** | `-target-tls insecure`（或 `verify` + `-target-tls-ca`） |

**先说清楚这个开关是干什么的**，因为很容易被当成安全特性：

浏览器到 agent 那一段是 WebRTC **强制**用 DTLS 1.3 加密的。所以这个开关解决的是**互操作性**——
让只肯说 TLS 的目标愿意跟你说话——而**不是**给整条链路加机密性（DTLS 已经在做了）。
"对面到底是谁"属于应用层（账号密码、token），不属于传输层。

| 值 | 含义 | 什么时候用 |
|---|---|---|
| `off`（默认） | 明文 | 目标说纯 HTTP（本机反代后面，最常见） |
| `insecure` | TLS，不验证证书 | 目标自签证书。**这是符合目的的默认，不是妥协** |
| `verify` | TLS + 验证（系统根 + `-target-tls-ca`） | 还想发现"连错服务" |

**这个坑真正难的地方是它的报错。** 两种服务器形态，两种毫无信息量的表现：

| 目标行为 | 浏览器看到 |
|---|---|
| 回一个 TLS alert（nginx / OpenResty 这类） | 502 `no header terminator` |
| 直接挂电话（Go 的 `tls.Listen` 这类） | 也是 502，只是**一个字节都没收到** |
| 用 301 强制跳 https | **重定向死循环**——跳过去的 `https://` 和外壳同 origin，SW 照样接管，又打回 TLS 端口 |

所以修法分两半，**两半都必要**：

1. **让它能通**：`-target-tls`，一个 flag。
2. **让配错时说得清楚**：识别 TLS 记录（`0x16`/`0x15`/`0x14` + `0x03`）和"零字节就关闭"两种情况，
   报出**该怎么做**，而不是报"没有头部终止符"。页面里直接显示：
